...
emojiLogger.log = function (msg, type, override) {
    ...
    var message = (type.displayEmoji ? emoji[type.icon] + " " : "") + msg;
 ...
 * @param {displayEmoji} boolean If `false`, emojis will not be added in the message. By default, it's `false` on Windows systems and `true` otherwise.
 * @return {Object} The type object.
 */
emojiLogger.add = function (type, icon, color, indent, stream, displayEmoji) {
    ...
    return emojiLogger.types[type] = {
      ...
      , displayEmoji: deffy(displayEmoji, !isWin() || !_icon)
    };
};
